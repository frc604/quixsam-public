...
class NTManager:
    ...
    def vision_received(self, table, key, value, is_new):
        try:
            ...
            elif np.isfinite(id_) and all(np.isfinite(value)) and len(value) == 17:
                self.vision_buffer.append(
                    [
                        Vision(id_, value[1], value[2]),
                        Vision(id_, value[5], value[6]),
                        Vision(id_, value[9], value[10]),
                        Vision(id_, value[13], value[14]),
                    ]
                )
    ...
    def publish_estimate(self, id_: int, x: float, y: float, theta: float):
        self.estimates_table.putNumberArray(str(id_), [float(id_), x, y, theta])
